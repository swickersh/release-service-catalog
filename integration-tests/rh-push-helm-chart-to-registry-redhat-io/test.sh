...
verify_release_contents() {
  ...
  do
    ...
    # Container images: plain skopeo inspect works. Helm OCI artifacts require --raw (same as release tasks).
    if skopeo inspect --tls-verify=true "docker://${COMPLETE_PULLSPEC}" &>/dev/null; then
    ...
    elif skopeo inspect --tls-verify=true --raw "docker://${COMPLETE_PULLSPEC}" 2>/dev/null | \
        ...
        echo "🔴 Failed to inspect '$COMPLETE_PULLSPEC' as container image or Helm OCI."
